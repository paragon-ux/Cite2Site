...
|---|---|---:|---|---|---|
| Authority | Clean archival boundary | Done | Archived package moved to `../Cite2Site-Archival/from-integration-gate9-finalization-b4b741d/`; old tracked docs/tests/examples removed from active repo; replacement docs, schemas, examples, and R0-R9 prompts exist; CI run `30347902903` passed. | None for R0. | Keep archived files out of the active repo. |
...
| Authority | Runtime rejects archived repositories | Done | `src/c2s/replacement.py`, `src/c2s/cli.py`, and `tests/test_replacement_archival_boundary.py`; active CLI no longer exposes old mutation commands; CI run `30347902903` passed. | None for R1 rejection. | Keep rejection fixture in active suite. |
