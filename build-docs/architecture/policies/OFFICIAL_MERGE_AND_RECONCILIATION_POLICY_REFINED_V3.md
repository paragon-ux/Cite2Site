...
### 1.1 Active protocol and mandatory archival boundary

This RFC defines the sole active Cite2Site authority model.

All earlier Cite2Site protocol, implementation, schema, compatibility, workflow, and stable-contract materials MUST be preserved as historical records rather than rewritten to imply they defined this model. Those materials MUST be placed outside the active authority reading order and MUST be clearly labelled **Historical**, **Superseded**, and **Unsupported by the active runtime**.

The active implementation MUST NOT migrate, reinterpret, reconcile, or transparently load authority created under an archived protocol. An archived-format repository MUST be rejected before replay, mutation, reconciliation, projection, or publication.

Reconciliation and import under this RFC apply only to repositories and operations that conform to this active authority model. Preserving archived documents and Git history MUST NOT create a backward-compatibility or conversion obligation.
...
- handle merge;
- reconciliation;
- any authority operation that derives or changes target fingerprints.
...
Reconciliation and import MUST reject archived-protocol authority before writes. They MUST NOT convert archived authority into this active model.
...
Mutation, batch mutation, and reconciliation MUST be mutually exclusive authority-writing operations within one repository.
...
Derived data MUST be regenerated after successful mutation or reconciliation.
...
Archived-protocol repositories MUST NOT be migrated or interpreted as current authority.

The implementation MUST reject an archived-format repository before replay or mutation and MUST NOT modify that repository while reporting the incompatibility.

Archived protocol documents, schemas, examples, compatibility materials, and source history MUST remain available as historical records, but MUST NOT appear in the active authority reading order or active conformance suite.
...
36. Archived-format repositories are rejected before replay or writes, and the archived authority package remains historically available outside the active conformance suite.
